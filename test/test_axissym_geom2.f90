      ...
      complex *16, allocatable :: rhs(:),sigma(:),potex(:)
      complex *16, allocatable :: Evals(:,:),Evalsex(:,:), &
        Hvals(:,:),Hvalsex(:,:)
      complex *16, allocatable :: potslp(:),potdlp(:),pot(:)
      ...
      real *8, allocatable :: pdis_rhab(:),pdis_lens(:),pdis_cone(:)
      real *8, allocatable :: rads_rhab(:),cms_rhab(:,:)
      real *8, allocatable :: rads_lens(:),cms_lens(:,:)
      real *8, allocatable :: rads_cone(:),cms_cone(:,:)
      real *8, allocatable :: wts_rhab(:),wts_cone(:),wts_lens(:)
...
!
      ...
      do ipatch=1,npatches
        rmin = wts(ixyzs(ipatch))
        do j=ixyzs(ipatch),ixyzs(ipatch+1)-1
          if(wts(j).le.rmin) rmin = wts(j)
        enddo
      enddo
...
!
      ...
      allocate(pdis_rhab(npatches_rhab),cms_rhab(3,npatches_rhab), &
        rads_rhab(npatches_rhab),wts_rhab(npts_rhab))

      call get_qwts(npatches_rhab,norders_rhab,ixyzs_rhab,iptype_rhab, &
        npts_rhab,srcvals_rhab,wts_rhab)
      call get_patch_distortion(npatches_rhab,norders_rhab,ixyzs_rhab, &
        iptype_rhab,npts_rhab,srccoefs_rhab,srcvals_rhab,wts_rhab, &
        pdis_rhab)
      call get_centroid_rads(npatches_rhab,norders_rhab,ixyzs_rhab, &
        iptype_rhab,npts_rhab,srccoefs_rhab,cms_rhab,rads_rhab)



      allocate(pdis_lens(npatches_lens),cms_lens(3,npatches_lens), &
        rads_lens(npatches_lens),wts_lens(npts_lens))

      call get_qwts(npatches_lens,norders_lens,ixyzs_lens,iptype_lens, &
        npts_lens,srcvals_lens,wts_lens)
      call get_patch_distortion(npatches_lens,norders_lens,ixyzs_lens, &
        iptype_lens,npts_lens,srccoefs_lens,srcvals_lens,wts_lens, &
        pdis_lens)
      call get_centroid_rads(npatches_lens,norders_lens,ixyzs_lens, &
      ...
      call get_patch_distortion(npatches_cone,norders_cone,ixyzs_cone, &
        iptype_cone,npts_cone,srccoefs_cone,srcvals_cone,wts_cone, &
        pdis_cone)
      call get_centroid_rads(npatches_cone,norders_cone,ixyzs_cone, &
        iptype_cone,npts_cone,srccoefs_cone,cms_cone,rads_cone)
      

      open(unit=44,file='rhab_data.txt')
      do i=1,npatches_rhab
        write(44,*) i,pdis_rhab(i),rads_rhab(i)
      enddo
      close(44)

      open(unit=44,file='cone_data.txt')
      do i=1,npatches_cone
        write(44,*) i,pdis_cone(i),rads_cone(i)
      enddo
      close(44)

      open(unit=44,file='lens_data.txt')
      do i=1,npatches_lens
        write(44,*) i,pdis_lens(i),rads_lens(i)
      enddo
      close(44)
