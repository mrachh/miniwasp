...
!
      nin= nz*nrin*ntin
      allocate(xyz_in(3,nin),charges_in(nin),xyz_out(3,nin))
      ...
      do iz=1,nz
        ...
        do ir=1,nrin
          ...
          do itt = 1,ntin
            ...
            xyz_in(3,ii) = z0*rsc
            xyz_out(1,ii) = rrout*cos(thet)*rsc
            xyz_out(2,ii) = rrout*sin(thet)*rsc
            xyz_out(3,ii) = z0*rsc
      ...
      enddo
      print *, "isrcin=",isrcin
      print *, "isrcout=",isrcout
      print *, "xyz_in=",xyz_in(1:3,isrcin)
      print *, "xyz_out=",xyz_out(1:3,isrcout)
      print *, "n_components=",n_components
      ...
      allocate(uval(npts),dudnval(npts),potslp(npts),potdlp(npts))
      allocate(pot(npts))

      zk = omega_use
      do i=1,npts
        call h3d_slp(xyz_out(1,isrcout),3,srcvals(1,i),0,dpars,1, &
          zk,0,ipars,uval(i))
        call h3d_sprime(xyz_out(1,isrcout),12,srcvals(1,i),0,dpars,1, &
          zk,0,ipars,dudnval(i))
      enddo
      zpars(1) = zk
      zpars(2) = 1.0d0
      zpars(3) = 0.0d0
      ...
      call lpcomp_helm_comb_dir(npatches,norders,ixyzs,iptype,npts, &
        srccoefs,srcvals,ndtarg,npts,srcvals,ipatch_id,uvs_targ,eps, &
        zpars,dudnval,potslp)
      zpars(1) = zk
      zpars(2) = 0.0d0
      zpars(3) = 1.0d0
