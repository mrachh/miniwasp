...
c
        if (err .gt. thresh/10) then
          call prin2('WARNING: Large error in chunk_dist1!*', x, 0)
          call prin2('t1 = *', t1, 1)
          call prin2('err = *', err, 1)
          call prin2('thresh = *', thresh, 1)
        endif
...
c
cccc          call prinf('ichunk = *', ichunk, 1)
cccc          call prin2('rtemp = *', rtemp(1,ichunk), k)
...
c
c
        subroutine chunklens(wgeo, rls, rltot)
        implicit double precision (a-h,o-z)
        double precision wgeo(1), rls(1)
        real *8 :: xymid(10), cmass(10)
c
        double precision, allocatable :: dsdt(:,:), rtemp(:,:)
        integer, allocatable :: adjs(:,:)
c
c       this routine returns in rls the arclength of each chunks
c
        done = 1
        call chunkunpack1(wgeo, k, nch, ichunks, iadjs, iders,
     1    iders2, ihs)
c
        do i = 1,nch
          call chunkaddr(i, wgeo, k, ichunk, iadj, ider,
     1        ider2, h)
          call chunksize(k, wgeo(ichunk), wgeo(ider), h, xymid,
     1       rad, cmass, crad, rls(i))
        enddo
c
        rltot = 0
        do i = 1,nch
...
c
c
        subroutine chunksort_scalar(k, nch, chunks, adjs, ders,
     1      ders2, hs, m, scalar)
        implicit real *8 (a-h,o-z)
        integer adjs(2,1)
        real *8 chunks(2,k,1),ders(2,k,1),ders2(2,k,1),hs(1)
        real *8 scalar(k,nch,m)
c
        real *8, allocatable :: c(:,:,:),d(:,:,:),d2(:,:,:),
     1      h(:), s(:,:,:)
c
...
c       is positively oriented)
c
c       also - this routine sorts scalar along with the chunks, m columns
c       at a time
c
c       this routine also assumes that chunks contains only one curve,
c       i.e. not two disjoint ones
...
c
c
        subroutine chunkzres(k, nch, chunks, ders, hs, sigma,
     1      errmax1, errmax2, tails)
        implicit real *8 (a-h,o-z)
        real *8 chunks(2,1,1),ders(2,1,1),hs(1),whts(1000),
     1      u(10000),v(10000),xs(1000),tails(1),xymid(10),
     2      cmass(10)
...
c     
        enddo
c
c       errmax1 is the approximate L2 error in sigma
c        
        errmax1 = sqrt(errmax1)
c
        return
        end
c
...
c
c
        subroutine chunkpt(inode, ich, wgeo, x, y)
        implicit real *8 (a-h,o-z)
        real *8 :: wgeo(*)
...
c
c
        subroutine chunkpoly_pack(eps, nverts, verts, ifclosed,
     1      ifdelete, nover, k, wgeo, lused)
        implicit real *8 (a-h,o-z)
        real *8 :: verts(2,nverts), wgeo(*)
c
        double precision, allocatable :: chunks(:,:,:), ders(:,:,:)
        double precision, allocatable :: ders2(:,:,:), hs(:)
        integer, allocatable :: adjs(:,:), itypes(:)
c
c       this routine constructs a chunked polygon, dyadically refining
c       near the edges, and automatically packs it all into wgeo, just
c       as the other routines do
c
c       input:        
c           eps - 
c           eps - 
...
c
        ...
        allocate(hs(maxchunks))
        allocate(adjs(2,maxchunks))
        allocate(itypes(maxchunks))
c
        call chunkpoly(eps, nverts, verts, ifclosed, ifdelete,
     1      nover, k, nch, chunks, adjs, ders, ders2, itypes, hs)
        
c
        if (nch .gt. maxchunks) then
          call prinf('in chunkpoly_pack, nch = *', nch, 1)
          call prinf('in chunkpoly_pack, maxchunks = *', maxchunks, 1)
          stop
        endif
c
        call chunkpack(k, nch, chunks, adjs, ders, ders2,
...
c             adjacency information for chunks near the corner will be -1
...
c
     ...
     2    ders2, hs)
        ...
        integer adjs(2,1)
        double precision chunks(2,k,1),ders(2,k,1), ders2(2,k,1),hs(1)
        double precision pars(1)
        real *8, allocatable :: xs(:),ws(:),xs2(:),ws2(:)
        real *8, allocatable :: u(:,:),v(:,:),u2(:,:),v2(:,:)
        real *8, allocatable :: ab(:,:),errs(:),errs0(:)
...
c
        maxchunks=25000
        allocate(ifprocess(maxchunks))
        do i=1,maxchunks
          ifprocess(i)=0
        enddo
        allocate(errs(1000),errs0(1000),ab(6,maxchunks))
...
c
        ...
        kkuse = 2*k
        allocate(xs2(kkuse),ws2(kkuse),u2(kkuse,kkuse))
        allocate(v2(kkuse,kkuse))
